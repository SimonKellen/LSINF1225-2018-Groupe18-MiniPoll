...
public class Sondage_Pour_Choix extends Poll {
    ...
    public Sondage_Pour_Choix(int id, String titre, Utilisateur createur, Utilisateur participants, Question question,String etat) {
        super.id = id;
        super.etat = etat;
        super.titre = titre;
    ...
    */
    ...
    public Question getQuestiondb() {
        SQLiteDatabase db = MySQLiteHelper.get().getReadableDatabase();
        String arg = Integer.toString(this.getId());
        Cursor cursor = db.rawQuery("SELECT \"ID_Question\", \"Numero_Ordre\", \"Format_Reponse\", \"Enonce\", \"ID_Poll\", \"Nbr_Choix\" from Questions Q INNER JOIN Poll P on Q.ID_Poll = P.ID_Poll WHERE (P.Type = \"S\") AND P.ID_Poll =" + arg, null);
        cursor.moveToFirst();
        Question q = null;
        while (!cursor.isAfterLast()) {
    ...
    public Reponse_Utilisateur getRepUti(){
        SQLiteDatabase db = MySQLiteHelper.get().getReadableDatabase();
        String arg = Integer.toString(this.getId());
        Cursor cursor = db.rawQuery("SELECT URep.ID_User, URep.ID_Reponse, URep.Score FROM UtilisateurRéponse URep, Poll, Questions, QuestionReponse WHERE Poll.Type = 'S' AND Poll.ID_Poll =" + arg + " AND Poll.ID_Poll = Questions.ID_Poll AND Questions.ID_Question = QuestionReponse.ID_Question AND QuestionReponse.ID_Reponse = URep.ID_Reponse",null );
        cursor.moveToFirst();
